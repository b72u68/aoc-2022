...
def get_data(filename):
    ...
    with open(file_dir, "r") as f:
        for line in f.readlines():
            q = [int(n) for n in re.findall(r"\d+", line)]
            data.append({
                ORE: {ORE: q[1]},
                CLAY: {ORE: q[2]},
...
# solution for part 1
def part1(data):
    return sum((i + 1) * largest_for_bp(bp, 24) for i, bp in enumerate(data))


# solution for part 2
def part2(data):
    return math.prod(largest_for_bp(bp, 32) for bp in data[:3])
